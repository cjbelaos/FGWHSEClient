...
{
    public partial class HOLMESLotUpdateHistory : System.Web.UI.Page
    {
        ...
        public DataTable getdetails()
        {
            ...
            DataSet ds = IHR.GET_HOLMES_LOT_MOVEMENT_HISTORY_LOGS(txtDateFrom.Text.Trim(), txtDateTo.Text.Trim(), txtRFIDTag.Text.Trim(), txtRefNo.Text.Trim(), txtLotNo.Text.Trim(), txtPartCode.Text.Trim(), ddlArea.SelectedValue.ToString());
        ...
        {
            Response.Redirect("HOLMESLotUpdateHistory.aspx");
        ...
        public void ExportToExcel()
        {
            ...
            //    string filename = "HOLMES_LOT_TRACE_" + DateTime.Now.ToString("ddMMyyyyhhmmssffffff");
            ...
            Response.AddHeader("content-disposition", "attachment;filename=HOLMES_LotTrace" + DateTime.Now.ToString("MMddyyyyHHmmssff") + ".xls");
            Response.Charset = "";
        ...
        {
            for (int i = grdDetails.Rows.Count - 1; i > 0; i--)
            {
                GridViewRow row = grdDetails.Rows[i];
                GridViewRow previousRow = grdDetails.Rows[i - 1];

                for (int j = 0; j < 1; j++)
                {

                    Label lblRef = (Label)row.Cells[j].FindControl("lblREFNO");
                    Label plblRef = (Label)previousRow.Cells[j].FindControl("lblREFNO");
                    Label lblLot = (Label)previousRow.Cells[j].FindControl("lblLOTNO");
                    Label plblLot = (Label)previousRow.Cells[j].FindControl("lblLOTNO");


                    if (lblLot.Text.ToUpper() == plblLot.Text.ToUpper())
                    {
                        if (lblRef.Text == plblRef.Text)
                        {
