...
{
    public partial class PSI_MASTER_VENDORS_CATEGORY : System.Web.UI.Page
    {
        ...
        [WebMethod]
        public static string GetSuppliersCategory(string strVENDOR)
        {
            PSIDAL psi = new PSIDAL();
            DataTable dt = psi.GET_TBL_M_SUPPLIER_CATEGORY(strVENDOR);

            return JsonConvert.SerializeObject(dt);
        }

        [WebMethod]
        public static string SaveSuppliersCategory(List<ObjectVendorsCategory> VC)
        {
            DALPSI_PLANT DPSI = new DALPSI_PLANT();
            string Message = "";
            foreach (var item in VC)
            {
                var strVENDOR = item.SupplierCode;
                var strCATEGORY = item.SupplierCategory;
                var strUSER = item.User;

                Message = DPSI.SAVE_SUPPLIER_CATEGORY(strVENDOR, strCATEGORY, strUSER);
            }
