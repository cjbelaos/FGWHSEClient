...
{
    public partial class PSIPartsSimulationAssemblyParts : System.Web.UI.Page
    {
        ...
        [WebMethod]
        public static string GetPartsSimulationAssemblyPartsByPlant(string strPLANT, string strPARTS, string strVENDORS)
        {
            DALPSI_PLANT DPSI = new DALPSI_PLANT();
            DataTable dt = DPSI.PSI_GET_PARTSSIMULATION_ASSEMBLY_BY_PLANT(strPLANT, strPARTS, strVENDORS);
            if (dt.Rows.Count > 0)
            {
                var StartDate = dt.Rows[0][214].ToString();
                for (var i = 16; i <= 213;)
                {
                    for (int j = 0; j <= 197; j++)
                    {
